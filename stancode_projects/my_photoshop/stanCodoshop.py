...
def get_best_pixel(pixels):
    ...
    # The closest_d is set as the distance of the first pixel, which will be replaced by smaller value if any.
    closest_d = get_pixel_dist(pixels[0], red, green, blue)
    pixel_closest = pixels[0]
    for pixel in pixels:
        color_distance = get_pixel_dist(pixel, red, green, blue)
        if color_distance < closest_d:
            pixel_closest = pixel
            closest_d = color_distance
    return pixel_closest
